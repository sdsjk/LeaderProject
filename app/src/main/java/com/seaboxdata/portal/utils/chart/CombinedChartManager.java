...
public class CombinedChartManager {
    ...
    private void initChart() {
        ...
        mCombinedChart.setTouchEnabled(false); // 所有触摸事件,默认true
        mCombinedChart.setDragEnabled(false);    // 可拖动,默认true
        mCombinedChart.setScaleEnabled(false);   // 两个轴上的缩放,X,Y分别默认为true
        mCombinedChart.setScaleXEnabled(false);  // X轴上的缩放,默认true
        mCombinedChart.setScaleYEnabled(false);  // Y轴上的缩放,默认true
        mCombinedChart.setPinchZoom(false);  // X,Y轴同时缩放，false则X,Y轴单独缩放,默认false
        mCombinedChart.setDoubleTapToZoomEnabled(false); // 双击缩放,默认true
        mCombinedChart.setDragDecelerationEnabled(true);    // 抬起手指，继续滑动,默认true
